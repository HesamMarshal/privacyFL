...
class ClientAgent(Agent):
    def __init__(self, agent_number, train_datasets, evaluator, active_clients):
        ...
        super(ClientAgent, self).__init__(
            agent_number=agent_number, agent_type="client_agent")
        ...
        # personal weights. Maps iteration (int) to weights (numpy array)
    ...
    def initializations(self):
        ...
        pubkeyList, seckeyList = dh.keygeneration(
            num_clients, self.agent_number)
        ...
        self.otherkeyList = {
            agent_name: None for agent_name, __ in clients.items()}
    ...
    def send_pubkeys(self):
        ...
        for agent_name, agent in self.directory.clients.items():
            # retrieve pubkey for client we're sending to
            pubkey = self.pubkeyList[agent_name]
            body = {'pubkey': pubkey}
            msg = Message(sender_name=self.name,
                          recipient_name=agent_name, body=body)
    ...
    def produce_weights(self, message):
        ...
        # iteration is indexed starting from 1
        if iteration - 1 > len(self.train_datasets):
            raise (ValueError(
        ...
        final_weights, final_intercepts = copy.deepcopy(
            weights), copy.deepcopy(intercepts)

        if config.USE_DP_PRIVACY:
            lock.acquire()  # for random seed
            final_weights, final_intercepts = \
                self.add_noise(weights=weights,
                               intercepts=intercepts, iteration=iteration)
            lock.release()

        if config.USE_SECURITY:  # adding security via diffie-helman offsets
            final_weights, final_intercepts = \
                self.add_security_offsets(
                    weights=final_weights, intercepts=final_intercepts)
        ...
        simulated_time += computation_time + \
            config.LATENCY_DICT[self.name]['server_agent0']
    ...
    def compute_weights_cumulative(self, iteration):
        ...
        if config.CLASSIFIER == 'Logistic':
            lr = SGDClassifier(
                alpha=0.0001, loss="log", random_state=config.RANDOM_SEEDS[self.name][iteration])
        elif config.CLASSIFIER == 'SVM':
            lr = SGDClassifier(alpha=0.0001, loss="hinge",
                               random_state=config.RANDOM_SEEDS[self.name][iteration])
        elif config.CLASSIFIER == 'Neural':
            lr = SGDClassifier(alpha=0.0001, loss="perceptron",
                               random_state=config.RANDOM_SEEDS[self.name][iteration])
    ...
    def compute_weights_noncumulative(self, iteration):
        ...
        if iteration > 1:
            federated_weights = copy.deepcopy(
                self.federated_weights[iteration - 1])
            federated_intercepts = copy.deepcopy(
                self.federated_intercepts[iteration - 1])
        else:
            federated_weights = None
            federated_intercepts = None
        # print(f"{len(y)=}  {pd.unique(y)=}")
        if config.VERBOSE_DEBUG:
            print(f"{self.agent_number=} {pd.unique(y)=}")

        lr.fit(X, y, coef_init=federated_weights,
               intercept_init=federated_intercepts)
        local_weights = lr.coef_
        local_intercepts = lr.intercept_
    ...
    def add_noise(self, weights, intercepts, iteration):
        ...
        active_clients_lens = [config.LENS_PER_ITERATION[client_name]
        ...
        for i in range(weights_shape[0]):  # weights_modified is 2-D
            for j in range(weights_shape[1]):
                if config.DP_ALGORITHM == 'Laplace':
                    dp_noise = laplace(
                        mean=config.mean, sensitivity=sensitivity, epsilon=epsilon)
                    ...
                    raise AssertionError(
                        'Need to specify config.DP_ALGORITHM as Laplace or Gamma')
        ...
        if config.INTERCEPTS_DP_NOISE:
            for i in range(intercepts_shape[0]):
                if config.DP_ALGORITHM == 'Laplace':
                    ...
                    scale = sensitivity / epsilon
                    num_clients = len(self.directory.clients)
                    dp_noise = random.gammavariate(
                        1 / num_clients, scale) - random.gammavariate(1 / num_clients, scale)
                else:
                    raise AssertionError(
                        'Need to specify config.DP_ALGORITHM as Laplace or Gamma')
                intercepts_dp_noise[i] = dp_noise

        # make a copy to not mutate weights
        weights_with_noise = copy.deepcopy(weights)
    ...
    def update_deltas(self):
        ...
        if None not in self.commonkeyList.values():  # if first time calling this function
            ...
            self.commonkeyList = self.commonkeyList.fromkeys(
                self.commonkeyList.keys(), None)
        else:
    ...
    def receive_weights(self, message):
        ...
        if config.USE_DP_PRIVACY and config.SUBTRACT_DP_NOISE:
            ...
            return_weights -= self.weights_dp_noise[iteration] / len(
                self.active_clients)
            return_intercepts -= self.intercepts_dp_noise[iteration] / len(
                self.active_clients)
        ...
        personal_accuracy = self.evaluator.accuracy(
            personal_weights, personal_intercepts)
        federated_accuracy = self.evaluator.accuracy(
            return_weights, return_intercepts)
    ...
    def satisfactory_weights(self, personal, federated):
        ...
        intercepts_differences = np.abs(
            federated_intercepts - personal_intercepts)
    ...
    def remove_active_clients(self, message):
        ...
        print('Simulated time for client {} to finish iteration {}: {}\n'.format(
            self.name, iteration, simulated_time))
